...
describe('duxkit-ui command shell', () => {
  it.each([
    ['init', ['--cwd', '/tmp/app', '--project', 'app', '--stylesheet', 'src/styles.scss', '--components-path', 'src/app/components/ai', '--style', 'scss', '--tokens', 'add', '--dry-run', '--json', '--yes', '--no-install']],
    ['add', ['message', 'conversation', '--cwd', '/tmp/app', '--project', 'app', '--components-path', 'src/app/components/ai', '--dry-run', '--json', '--yes', '--no-install', '--force']],
    ['list', ['--cwd', '/tmp/app', '--json']],
  ...
  it('fails clearly for unsupported option values', async () => {
    ...
    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain("error: option '--tokens <mode>' argument 'replace' is invalid");
  });
