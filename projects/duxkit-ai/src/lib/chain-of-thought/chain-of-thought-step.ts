...
import { ContentClamp } from 'duxkit-ai/reasoning';
...
@Component({
  ...
  template: `
    @if (layout(); as customLayout) {
      <ng-container
        [ngTemplateOutlet]="customLayout"
...
export class ChainOfThoughtStep extends ContentClamp {
  /** Replace the complete step layout. Template receives the step state as $implicit. */
  public readonly layout = input<TemplateRef<{ $implicit: ChainOfThoughtStep }>>();
  ...
  public readonly defaultIcon = computed(() => statusIcons[this.status()]);
  public readonly spinning = computed(() => this.defaultIcon() === 'lucideLoaderCircle');
