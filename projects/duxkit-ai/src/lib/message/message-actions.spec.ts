...
import { MessageActions, MessageCopy, MessageThumbsDown, MessageThumbsUp } from './';
...
@Component({
  imports: [Message, MessageActions, MessageCopy, MessageContent],
  template: `
    <ai-message from="assistant">
      <ai-message-content markdown="Registered text" />
      <ai-message-actions>
        <button aiMessageCopy text="Explicit text" aria-label="Copy explicit text">Copy</button>
      </ai-message-actions>
    </ai-message>
  `,
})
...
describe('MessageActions', () => {
  ...
  it('applies actions wrapper defaults and preserves projected custom actions', () => {
    ...
    const copy = element.querySelector('button[aiMessageCopy]');
  ...
  it('prefers explicit copy text when provided', async () => {
    ...
    const copy = element.querySelector<HTMLButtonElement>(
      'button[aria-label="Copy explicit text"]',
    );
