...
@Component({
  ...
  template: `
    @if (hasProjectedPreview()) {
      <ng-content select="[aiAttachmentThumbnail],ai-attachment-thumbnail" />
      <ng-content select="[aiAttachmentName],ai-attachment-name" />
      <ng-content select="[aiAttachmentMediaType],ai-attachment-media-type" />
    } @else {
...
export class AttachmentPreview {
  ...
  protected readonly projectedThumbnail = contentChild(AttachmentThumbnail);
  protected readonly projectedName = contentChild(AttachmentName);
  protected readonly projectedMediaType = contentChild(AttachmentMediaType);

  /** Additional classes merged onto the attachment preview element. */
  public readonly userClass = input<string | undefined>(undefined, { alias: 'class' });

  protected readonly hasProjectedPreview = computed(
    () =>
      this.projectedThumbnail() !== undefined ||
      this.projectedName() !== undefined ||
      this.projectedMediaType() !== undefined,
  );
